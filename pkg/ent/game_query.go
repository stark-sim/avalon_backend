...
func (gq *GameQuery) QueryGameUsers() *GameUserQuery {
	query := &GameUserQuery{config: gq.config}
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withGameUsers; query != nil {
		if err := gq.loadGameUsers(ctx, query, nodes,
			func(n *Game) { n.Edges.GameUsers = []*GameUser{} },
			func(n *Game, e *GameUser) { n.Edges.GameUsers = append(n.Edges.GameUsers, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GameQuery) loadGameUsers(ctx context.Context, query *GameUserQuery, nodes []*Game, init func(*Game), assign func(*Game, *GameUser)) error {
	...
	query.Where(predicate.GameUser(func(s *sql.Selector) {
		s.Where(sql.InValues(game.GameUsersColumn, fks...))
	}))
