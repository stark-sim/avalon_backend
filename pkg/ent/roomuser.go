	...
	for i := range columns {
		...
		case roomuser.FieldCreatedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				ru.CreatedBy = value.Int64
			}
		case roomuser.FieldUpdatedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field updated_by", values[i])
			} else if value.Valid {
				ru.UpdatedBy = value.Int64
			}
...
func (ru *RoomUser) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", ru.CreatedBy))
	builder.WriteString(", ")
	builder.WriteString("updated_by=")
	builder.WriteString(fmt.Sprintf("%v", ru.UpdatedBy))
	builder.WriteString(", ")
